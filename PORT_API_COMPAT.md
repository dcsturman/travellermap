...
**Next — needs a decision (the "others"):** the remaining documented endpoints either **collide with our private contract** (`/api/universe`, `/api/search` already exist with snake_case shapes our Leptos client depends on) or **need new index/writer work** (`/api/sec` text via `SectorWriter`; `/api/metadata` + `/api/credits` via a fuller metadata parse; `/api/jumpworlds` + `/api/route` via a cross-sector spatial index — `route` partly exists; `/api/msec` via `MSECWriter`). The core question from the headline finding is still open: **reshape `/api/universe` & `/api/search` to PascalCase (breaking our client, which we'd then migrate)** vs **serve the compat shape at a parallel path and keep the private one**. Discuss before proceeding.
