...
| `GET /api/credits?sector=&hex=` (+`sx/sy/hx/hy`,`x/y`) • `/data/{sector}/credits` • `…/{hex}/credits` | Per-location world data + sector credits/author/source | `CreditsHandler.cs` | Data | **DONE** (JSON) | `get_credits` reuses the shared `assemble_metadata` parse (sector `.xml` over inline region block) for Credits/Author/Source/Publisher/Copyright/Tags/Products + subsector (by hex→letter) + world lookup at the hex. Hex defaults to 1620 (`SectorCentralHex`). Null fields omitted, matching the reference serializer. Verified exact vs live for Spinward Marches 1910. Remaining: XML/JSONP output, `/data/{sector}/credits` alias, `RouteCredits`/`SubsectorCredits`/`WorldCredits` (always empty in current data). |
| `GET /api/route?start=&end=&jump=` (+`wild`,`im`,`nored`,`aok`,`x/y`,`milieu`) | Shortest jump route between two worlds | `RouteHandler.cs` | Data | **DONE** (JSON) | `get_route` over the cross-sector route index (constrained pathfinder: refuel/Imperial/red-zone/anomaly). Default emits the documented public **bare array of `RouteStop`** (Sector/SectorX/SectorY/Name/Hex/HexX/HexY/UWP/PBG/Zone/AllegianceName), `SectorX/Y`+`HexX/Y` recovered from each stop's absolute coord. Private `&detail=true` returns the rich `{waypoints,jumps,parsecs}` (absolute coords) the Leptos client draws from. Remaining: `Subsector` name per stop (route index holds no subsector names — omitted), XML/JSONP output. |
