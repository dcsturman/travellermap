...
use super::common::{
    hex_parsec, on_screen, sector_center, world_hex, ViewState, DEFAULT_FONT, SECTOR_H, SECTOR_W,
    SUBSECTOR_H, SUBSECTOR_W,
};
use super::Theme;
...
pub(crate) fn draw_subsector_names(
    c: &impl Canvas,
    view: &ViewState,
    w: f64,
    h: f64,
...
pub(crate) fn draw_border_labels(
    c: &impl Canvas,
    view: &ViewState,
    w: f64,
    h: f64,
    ...
    for border in &sector.borders {
        ...
        let (px, py) = (
            px + border.label_offset.0 as f64 * 0.7,
            py - border.label_offset.1 as f64 * 0.7,
        );
...
pub(crate) fn draw_sector_labels(
    c: &impl Canvas,
    view: &ViewState,
    w: f64,
    h: f64,
    ...
    for label in &sector.labels {
        ...
        let (px, py) = hex_parsec(wc, wr);
        let (px, py) = (
            px + label.offset.0 as f64 * 0.7,
            py - label.offset.1 as f64 * 0.7,
        );
        let (x, y) = view.to_screen(w, h, (px, py));
        if !on_screen(x, y, w, h, size * 4.0) {
            continue;
